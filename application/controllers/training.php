...
class Training extends CI_Controller {
			...
			if($this->db->count_all_results('mw_pages') > 0)
			{
				...
				foreach($pages->result() as $page)
				{
					if($identifier == $page->url)
						$sidebar['sidebar'] .= '<li class="activex"><a href="training/page/' . $page->url . '">'. $page->title .'</a></li>';
					else
						$sidebar['sidebar'] .= '<li><a href="training/page/' . $page->url . '">'. $page->title .'</a></li>';
				}
	...
	public function page($identifier)
	{
		...
		switch($template)
		{
			...
			case 'gallery.php':
			$this->db->where('school', 5);
			$data['school_url']='training';
