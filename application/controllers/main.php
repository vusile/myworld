...
class Main extends CI_Controller {
	...
	{
		$header['title'] = "My World Pre School";
	...
	public function get_projects($section)
	{
		...
		$parent_categories = $this->db->get('mw_projects_and_publications');
		
		$this->db->where('parent_category >',0);
		$this->db->where('section',$section);
		$this->db->order_by('parent_category');
		$child_categories = $this->db->get('mw_projects_and_publications');
	...
	public function nl($url)
	{
		...
		$data['stories'] = $this->db->get('mw_news');
